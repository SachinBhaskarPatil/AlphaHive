...
async def retrieval_node(state: AgentState) -> dict:
    ...
    if not raw_tickers:
        ...
        try:
             ...
             portfolio_data = read_portfolio.run("Portfolio.csv")
    ...
    # Best-effort LTM lookup — the graph must still run when PostgreSQL is
    # unavailable (local/dev/Docker without a DB), so we guard on try_connect()
    # exactly like the shadow/profile/reporting nodes do.
    if await db.try_connect():
        for ticker in clean_tickers:
            past_call = await db.get_memory(user_handle, ticker)
            if past_call and past_call.get('analysis_date') is not None:
                historical_stances[ticker] = {
                    "last_stance": past_call.get('stance'),
                    "last_logic": past_call.get('logic_summary'),
                    "date": past_call['analysis_date'].isoformat(),
                }
    else:
        logger.warning("Skipping LTM lookup — database unavailable")
