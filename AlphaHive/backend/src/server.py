...
# Shared LLM config (root config module — loaded after ROOT_DIR is on sys.path)
try:
    from config import LLM_PROVIDER, LLM_FLASH_MODEL, LLM_PRO_MODEL, OPENAI_API_KEY
    _llm_configured = LLM_PROVIDER == "openai" and bool(OPENAI_API_KEY)
    logger.info("LLM provider: %s (flash=%s, pro=%s)", LLM_PROVIDER, LLM_FLASH_MODEL, LLM_PRO_MODEL)
except ImportError:
    LLM_PROVIDER = os.getenv("LLM_PROVIDER", "openai")
    _llm_configured = bool(os.getenv("OPENAI_API_KEY"))
    logger.warning("Root config.py not found — using env vars for LLM settings")
