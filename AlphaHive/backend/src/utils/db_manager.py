...
class DBManager:
    ...
    async def try_connect(self) -> bool:
        """Connect if possible; return False instead of raising (local dev without Postgres)."""
        try:
            await self.connect()
            return True
        except Exception as e:
            logger.warning("PostgreSQL unavailable — using JSON/file fallback: %s", e)
            return False
    ...
    async def list_memories(self, user_handle: str | None = None, limit: int = 50):
        """Recent LTM rows for Agent Brain (optionally scoped to one user)."""
        limit = max(1, min(int(limit or 50), 200))
        async with self.pool.acquire() as conn:
            if user_handle:
                return await conn.fetch(
                    """
                    SELECT user_id, ticker, stance, logic_summary, analysis_date
                    FROM public.agent_memory_ltm
                    WHERE user_id = $1
                      AND UPPER(ticker) <> 'PORTFOLIO'
                    ORDER BY analysis_date DESC
                    LIMIT $2
                    """,
                    user_handle,
