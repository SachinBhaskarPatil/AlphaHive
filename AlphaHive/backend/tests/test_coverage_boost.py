...
def test_forecast_returns_list():
    """GET /api/forecasts/<handle> returns {forecasts, stances} — stances from DB."""
    mock_rows = [
        {
            "ticker": "TCS.NS",
            "stance": "BULLISH",
    ...
    with patch.object(db, "try_connect", AsyncMock(return_value=True)), \
        ...
        assert data["stances"][0]["ticker"] == "TCS.NS"


def test_forecast_empty_history():
    """No DB stances → forecasts/stances both empty lists, still HTTP 200."""
    with patch.object(db, "try_connect", AsyncMock(return_value=False)):
...
def test_forecast_db_error_returns_500():
    """Unexpected failure inside the handler surfaces as HTTP 500."""
    with patch.object(db, "try_connect", AsyncMock(side_effect=Exception("DB down"))):
        res = client.get("/api/forecasts/test@gmail.com")
...
def test_heatmap_exception_returns_500():
    # Profile GET surfaces unexpected errors as HTTP 500 (try_connect raising).
    with patch.object(db, "try_connect", AsyncMock(side_effect=Exception("db crash"))):
...
def test_profile_get_no_db():
    """GET /api/profile/<handle> degrades gracefully when DB is unavailable."""
    with patch.object(db, "try_connect", AsyncMock(return_value=False)):
...
def test_profile_get_no_persona():
    with patch.object(db, "try_connect", AsyncMock(return_value=False)):
        ...
        assert res.status_code == 200
        # New users with no stored profile default to the "Not Set" sentinel,
        # which the frontend uses to trigger onboarding.
        assert res.json()["persona"] == "Not Set"
...
async def test_sentiment_node_bearish():
    ...
    with patch("yfinance.Ticker") as mock_yf:
        ...
        assert result["sentiment_data"][0]["sentiment"] == "negative"


@pytest.mark.asyncio
async def test_sentiment_node_nested_content_title():
    """Yahoo Finance currently nests headline under content.title."""
    from src.agents.sentiment_node import sentiment_node
    with patch("yfinance.Ticker") as mock_yf:
        mock_yf.return_value.news = [
            {"id": "1", "content": {"title": "Stock posts strong profit growth and high gains"}},
            {"id": "2", "content": {"title": "Expansion plans and positive outlook"}},
        ]
        result = await sentiment_node({
            "tickers": ["HDFCLIFE.NS"],
            "regime": "NEUTRAL",
        ...
        assert result["sentiment_data"][0]["sentiment"] == "positive"
        assert "profit" in result["sentiment_data"][0]["summary"].lower()
