...
@pytest.mark.asyncio
async def test_retrieval_node_degrades_without_db(base_state):
    """When PostgreSQL is unavailable the node still standardizes tickers and
    returns an empty LTM map instead of raising."""
    with patch("src.agents.retrieval_node.db") as mock_db:
        mock_db.try_connect = AsyncMock(return_value=False)
        mock_db.get_memory = AsyncMock(side_effect=AssertionError("must not query DB when disconnected"))

        result = await retrieval_node(base_state)

        assert "TCS.NS" in result["tickers"]
        assert result["historical_stances"] == {}
